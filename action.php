<?php
	...
	if (isset($_POST['submit1'])){
		// setting nama folder upload
		$uploaddir = 'photos/';
		// waktu dan tanggal upload
		$uploadtime = date('YmdHis');
		// setting sesuai ketentuan (namauser-tanggal.ext)
		$uploadfile = $_POST['username']."-".$uploadtime.".".pathinfo($_FILES['userfile']['name'],PATHINFO_EXTENSION);
		// proses upload ke data
		if (move_uploaded_file($_FILES['userfile']['tmp_name'], $uploaddir.$uploadfile)){
			echo "File berhasil diupload";
			} else{
				echo "Maaf, file gagal diupload";
			}
			// set session nama untuk peng-uploadan ke db
